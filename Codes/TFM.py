...
###############################################################################
x_trans = []
y_trans = np.zeros(len(range(0, N_element, 3)), )

start_recording_poss_x = (W_sample - (N_element - 1.0) * pitch_element) / 2.0

for i in range(0, N_element, 3):
...
im = plt.scatter(nodes_interest_x * 1e3, nodes_interest_y * 1e3, s =100, c = I_global_final / np.max(I_global_final), cmap = my_cmap, zorder = 2)
